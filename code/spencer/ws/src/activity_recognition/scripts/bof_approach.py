...
# BOF 128 clusters, 100 frames, 50 samples for each testlabel, 88, 88
# pred
...
def plot_confusion_matrix(gt, pred):
    ...
    cax = ax.matshow(cm, cmap=pylab.cm.jet)
    fig.colorbar(cax)

    for i, cas in enumerate(cm):
        for j, c in enumerate(cas):
            if c>0:
                plt.text(j, i, c, fontsize=15, 
    ...
    for testSample, testLabel in zip(testSamples, testLabels):
        ...
        if test_partial:
            for i in range(50):
                ...
                testSampleInterval = testSample[start_frame:start_frame+frames]
                ground_truth.append(testLabel)
                prediction.append(predict(classificator, nn_tree, testSampleInterval)[0])
