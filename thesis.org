...
- high computational complexity ::
     A run-time complexity of $\mathcal{O}(n^3)$ for the training is a serious drawback, as it is not practical to use /Gaussian Process/ with data which has many samples. 
