...
class SequenceMapping(Mapping):
    ...
    def __init__(self, Y, input_dim=1, output_dim=1):
    ...
    def _get_param_names(self):
        return ['a_%i' % s for s in range (self.sequences)]
    ...
    def f(self, X):
        return sum([ a[s] * exp(self.dtw.compute(X, Y[s])) for s in range(self.sequences)])

    def df_dtheta(self, dL_df, X):
        self._df_da = (dL_df[:, :, None]*[exp(self.dtw.compute(X,Y[s])) for s in range(self.sequences)] ).sum(0).T
...
class Constraints(Mapping):
    ...
    def __init__(self, Y, sequences, input_dim, output_dim=2):
        self.sequences = sequences.append(Y.shape[0])
        self.Y = Y
    ...
    def _get_param_names(self): 
        return sum([['Lambda_%i_%i' % (s, q) for q in range(output_dim)] for s in range(seq_num)], 
                   [['A_%i_%i' % (s, q) for q in range (output_dim)] for s in range(seq_num)])

    def _get_params(self):
        return np.hstack((self.Lambda.flatten(), self.A.flatten()))

    def _set_params(self, x):
        self.Lambda = x[:self.seq_num*self.output_dim].reshape(self.seq_num, self.output_dim).copy()
        self.A = x[:self.seq_num*self.output_dim].reshape(self.seq_num, self.output_dim).copy()

    def randomize(self):
        self.Lambda = nm.random.randn(self.seq_num, self.output_dim)/np.sqrt(self.seq_num+1)
        self.A = nm.random.randn(self.seq_num, self.output_dim)/np.sqrt(self.seq_num+1)

    def g(Y_s, q):
        return sum(A[s,q] * exp(self.dtw.compute(Y_s, self.Y_s(s)) for s in range(seq_num)))

    def Y_s(self, s):
        return Y[sequences[s]:sequences[s+1]]

    def X_s(self, X, s):
        return X[sequences[s]:sequences[s+1]]

    def df_dtheta(self, dl_df, X):
        return sum([[g(Y_s(s) - np.mean(X_s(X,s))) for q in range(output_dim)] for s in range(seq_num)], [])
...
*** Sequence back-constrained GP-LVM
#+begin_src python
...
class SeqBCGPLVM(GPLVM):
    ...
    """
    def __init__(self, Y, input_dim, init='PCA', X=None, kernel=None, normalize_Y=False, seq_index):
        
        self.seq_index = seq_index
        self.constraints = SeqConstraints(Y, seq_index)
    ...
    def _get_param_names(self):
        return self.constraints._get_param_names() + GP._get_param_names(self)

    def _get_params(self):
        return np.hstack((self.constraints._get_params(), GP._get_params(self)))

    def _set_params(self, x):
        self.constraints._set_params(x[:self.mapping.num_params])
    ...
    def _log_likelihood_gradients(self):
        ...
        dL_dtheta = self.constraints.df_dtheta(dL_df, self.likelihood.Y)
        dL_dX = self.constraints.df_dX(dL_df, self.likelihood.Y)
        return np.hstack((dL_dX.flatten(), dL_dtheta.flatten(), GP._log_likelihood_gradients(self)))
