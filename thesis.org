...
In the paper "Discriminative Sequence Back-Constrained GP-LVM for MOCAP Based Action Recognition"ebib:_discriminative_2013 the authors propose a method for classifying MOCAP actions. By using a similarity feature for the sequences in the observed space and constraining the optimization to preserve this measure the local distances between the sequences are transferred into the latent space. This has two advantages. First of all the sequences have a meaningful clustering in the latent space. Second by also learning the back-constraint it is possible to calculate the centroid of a sequence in the latent space directly without maximizing a likelihood. This in turn is being used to do real-time classification for actions. The mapping is defined as a linear combination of the DTW distance between every other sequence. For every latent dimension $q$ we have: 
...
class SequenceMapping(Mapping):
    ...
    def __init__(self, input_dim=1, output_dim=1, sequences=2, sequence_indices=[0]):
        self.name = 'sequence_constraint'
        Mapping.__init__(self, input_dim=input_dim, output_dim=output_dim)
        self.num_params = self.output_dim*self.sequences
        self.A = np.array((self.sequences, self.output_dim))
    ...
    def _get_param_names(self):
        return sum([['A_%i_%i' % (n, d) for d in range(self.sequences)] for n in range(self.output_dim)], [])

    def _get_params(self):
        return np.hstack((self.A.flatten()))

    def _set_params(self, x):
        self.A = x[:self.sequences * self.output_dim].reshape(self.sequences, self.output_dim).copy()

    def randomize(self):
        self.A = np.random.randn(self.sequences, self.output_dim)/np.sqrt(self.sequences)

    def f(self, X):
        dtw_class = mlpy.Dtw()
        
        [[for s in range(self.sequences)] for q in range(self.output_dim)]
    ...
    def df_dX(self, dL_df, X):
        return (dL_df[:, None, :]*self.A[None, :, :]).sum(2) 
